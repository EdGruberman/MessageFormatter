...
public class BufferedYamlConfiguration extends YamlConfiguration implements Runnable {
    ...
    public void save() {
        try {
            ...
            this.plugin.getLogger().severe("Unable to save configuration file: " + this.file + "; " + e);
            return;
        ...
        if (elapsed < this.minSave) {
            ...
            if (this.isQueued()) {
                this.plugin.getLogger().log(Level.FINEST
                        , "Queue request already will run in {0} seconds for configuration file: {1} (Last save was attempted {2} seconds ago)"
            ...
            this.plugin.getLogger().log(Level.FINEST
                    , "Queued save request to run in {0} seconds for configuration file: {1} (Last save was attempted {2} seconds ago)"
                    , new Object[]{delay / 1000, this.getFile(), elapsed});
            return;
        }
